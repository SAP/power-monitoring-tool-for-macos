 ...
 *                  <code>IOPMAssertionCreateWithProperties</code> describing dictionary format.
 *
 ...
 */
CFDictionaryRef IOPMCopyAssertionActivityAggregate(void);
 ...
 *                  kIOReturnUnsupported if the setting isnÍt supported on this hardware.
 */
...
/*!
 ...
 @param             energyPrefs A CFMutableDictionaryRef containing the energy preferences.
 */
 ...
 * @param queue            The dispatch_queue on which the registered block is called.
 *
 * @param inblock       The block to execute when remote node power state changes.
 *                  The block will receive two arguments:
...
IOReturn IOPMEnableAsyncAssertions(void);
IOReturn IOPMDisableAsyncAssertions(void);
